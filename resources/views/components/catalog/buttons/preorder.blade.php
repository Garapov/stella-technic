<div class="relative w-full md:w-auto" x-data="{ showTooltip: false }">
    <button type="button"
        @click.stop="@if (setting('open_preorder')) {{ setting('open_preorder') }} @endif $store.application.forms.preorder = true; $store.application.one_click_variation = {{json_encode($variant)}}"
        @mouseenter="showTooltip = true" @mouseleave="showTooltip = false"
        class="inline-flex items-center justify-center rounded-lg px-5 py-2.5 text-sm font-medium text-white focus:outline-none focus:ring-4 w-full md:w-auto bg-blue-500 hover:bg-blue-800 focus:ring-blue-300">
        <x-carbon-shopping-cart-plus class="h-5 w-5" />
        <span class="md:sr-only ms-2">Предзаказ</span>
    </button>
