<div class="p-3 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-900 dark:border-gray-700 flex flex-col gap-3"
    wire:loading.class="opacity-50 pointer-events-none" x-data="{
    isOpened: true
}">
    @php
    ...
    <ul class="space-y-2 text-sm" aria-labelledby="dropdownDefault" x-data="{ showAll: false }" x-show="isOpened">
        ...
        @if (count($paramGroup) > 5)
            <li @click="showAll = !showAll"
                class="cursor-pointer font-medium text-blue-600 dark:text-blue-500 hover:underline">
                Показать <template x-if="!showAll"><span>больше</span></template><template
                    x-if="showAll"><span>меньше</span></template>
            </li>
        @endif
    </ul>
</div>