...
<div x-data="{
    activeTabIndex: 0
}" x-init="isLoaded = true" class="flex flex-col md:mb-10">
    @if (count($categories) > 0)
        <div class="flex items-center justify-between md:overflow-x-hidden overflow-x-auto" x-cloak>
            <ul class="flex md:flex-wrap text-sm font-medium text-center text-gray-500 dark:text-gray-400">
                @php
                    $counter = 0;
                @endphp
                @foreach($categories as $key => $category)
                    ...
                    @endif
                    <li class="me-2">
                        <span class="inline-block p-4 rounded-t-lg border border-blue-500 whitespace-nowrap md:whitespace-auto" :class="activeTabIndex == {{$counter}} ? 'text-white bg-blue-500 active ' : 'text-gray-900 bg-slate-50 cursor-pointer'" @click="activeTabIndex = {{$counter}}">{{ $category->title }}</span>
                    </li>
        ...
        @foreach($categories as $key => $category)
            ...
            <div class="flex flex-col gap-4 p-4 border border-slate-200 rounded-b-lg shadow-sm bg-slate-50 rounded-b-lg mt-[-1px]" x-show="activeTabIndex == {{ $counter }}" x-cloak>
                ...
                <div class="grid xl:grid-cols-6 lg:grid-cols-4 md:grid-cols-3 grid-cols-2 md:gap-4 gap-2">
                    @foreach($category->categories as $key => $subcategory)
                        <a href="{{ route('client.catalog', $subcategory->urlChain()) }}" class="p-2.5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-slate-200 shadow-sm hover:shadow-lg shadow-slate-50/50 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 flex flex-col-reverse md:flex-row items-center justify-between gap-2">
                            ...
                            @if (!empty($subcategory->image))
                                <img class="md:min-w-10 md:max-w-10 md:min-h-10 md:max-h-10 w-[80%] mx-auto object-cover" alt="{{ $subcategory->title }}" loading="lazy" class="" src="{{ Storage::disk(config('filesystems.default'))->url($subcategory->image) }}">
                            @endif
                        </a>
                    @endforeach
                </div>
