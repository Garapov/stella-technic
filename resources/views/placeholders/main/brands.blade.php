<div class="bg-slate-50 py-10 xl:px-[100px] px-[20px]">
    ...
    <div class="grid justify-between gap-4 grid-cols-1 md:grid-cols-3 lg:grid-cols-6 text-gray-400">
        @foreach (range(1,6) as $key=>$i)
            <div class="rounded  p-4 justidy-center bg-slate-100 rounded-sm lg:rounded-xl animate-pulse h-20 @if ($key < 1) flex md:flex @endif @if ($key > 0 && $key < 3) hidden md:flex @endif @if ($key >= 3) hidden lg:flex @endif  ">
