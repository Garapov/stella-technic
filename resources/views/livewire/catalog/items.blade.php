                        ...
                        <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900 mb-4">
                            <ul class="space-y-2">
                                @foreach ($category->categories as $subcategory)
                                    <li x-data="{ category{{$subcategory->id}}Opened: false }">
                                        @if (!count($subcategory->categories))
                                            <a href="{{ route('client.catalog', $subcategory->slug) }}" class="flex items-center p-2 w-full text-base font-normal text-gray-900 rounded-lg transition duration-75 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" wire:navigate>
                                        @else
                                            <div class="flex items-center p-2 w-full text-base font-normal text-gray-900 rounded-lg transition duration-75 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" @click.stop="category{{$subcategory->id}}Opened = !category{{$subcategory->id}}Opened">
                                        @endif
                                            <img src="{{ asset('storage/'. $subcategory->image) }}" class="w-6" alt="">
                                            

                                            <span class="flex-1 ml-3 text-left whitespace-nowrap">{{ $subcategory->name }}</span>
                                        ...
                                        @endif
                                        @if (count($subcategory->categories))
                                            <ul class="py-2 space-y-2" x-show="category{{$subcategory->id}}Opened">
                                                @foreach ($subcategory->categories as $subsubcategory)
                                                    <li>
                                                        <a href="{{ route('client.catalog', $subsubcategory->slug) }}" class="flex items-center p-2 pl-11 w-full text-base font-normal text-gray-900 rounded-lg transition duration-75 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" wire:navigate>{{ $subsubcategory->name }}</a>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        @endif
