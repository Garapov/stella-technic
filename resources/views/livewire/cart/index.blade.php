                            ...
                            <div class="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                              <div class="flex items-center gap-4">
                                  ...
                                  <div class="flex flex-col gap-4">
                                    ...
                                    <div class="flex items-center gap-4">
                                      <template x-for="(colors, size) in construct.boxes" :key="size">
                                        <template x-for="(box, color) in colors" :key="color">
                                          <template x-if="box.count > 0">
                                            <div class="flex flex-col gap-2 relative">
                                              <img class="h-20 w-20"
                                                  :src="(box.product?.gallery?.length ?? 0) ? `https://s3.stella-technic.ru/${box.product.gallery[0]}` : default_image"
                                                  :alt="`${size} ${color}`" />

                                              <span class="bg-blue-200 text-xs font-medium text-blue-800 text-center p-0.5 leading-none rounded-full px-2 dark:bg-blue-900 dark:text-blue-200 absolute -translate-y-1/2 translate-x-1/2 left-auto top-0 right-0"
                                                    x-text="box.count"></span>
                                            </div>
                                          </template>
                                        </template>
                                      </template>
