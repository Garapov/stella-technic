<form action="{{ route('client.search') }}" class="relative w-full" x-data="{
    ...
    <div class="rounded-lg bg-blue-500 flex items-center relative w-full z-20">
        <input type="search" id="search-dropdown"
            ...
            placeholder="Поиск" name="q" x-model="query" @input.debounce.500ms="findResults" @focus="isOpen = true"
            @input="isOpen = true" style="outline: none; box-shadow: none;" autocomplete="off" />
        <button type="submit"
            ...
            <template x-if="isLoading">
                <div role="status">
                    <svg aria-hidden="true" class="w-5 h-5 text-blue-500 animate-spin dark:text-gray-600 fill-white"
    ...
    <div class="absolute top-full left-0 right-0 bg-white border border-blue-500 pt-8 pb-4 px-4 max-h-[400px] overflow-y-scroll rounded-lg -mt-4 z-10 flex flex-col gap-4"
        ...
        @if ($q == '' && $results['products']->count() < 1)
            <span class="text-md font-semibold" x-show="JSON.parse(JSON.stringify(searchRequests)).length < 1">Вы пока
                ничего не искали...</span>
        @endif

        @if ($q != '' && $results['products']->count() < 1)
            <div class="flex items-center p-4 text-sm text-yellow-800 border border-yellow-300 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300 dark:border-yellow-800"
                role="alert">
        ...
        <ul x-show="JSON.parse(JSON.stringify(searchRequests)).length > 0 && query == ''"
            ...
            <template x-for="(searchRequest, key) in JSON.parse(JSON.stringify(searchRequests)).reverse()"
                :key="searchRequest">
                <li class="flex justify-between items-center py-2 px-0 hover:shadow-md hover:px-2 hover:bg-white rounded-xl transition-all cursor-pointer"
                    @click="setQueryFromHistory(searchRequest)" x-show="key < 5">
                    ...
                    @foreach($results['products']->groupBy(function ($prod) {
                        ...
                        <div>
                            @foreach ($productGroup as $product)
                                <a href="{{ route('client.catalog', $product->urlChain()) }}" wire:navigate
                                    class="flex items-center gap-4 py-2 px-0 hover:px-2 rounded-lg hover:bg-white hover:shadow-md transition-all">
                                    {{-- <img src="https://placehold.co/100x100" alt="Product"
                                        class="w-20 h-20 object-cover rounded-md" /> --}}
                                    @if ($product->gallery && count($product->gallery) > 0)
                                        <img class="w-20 h-20 object-cover rounded-md"
                                            src="{{ Storage::disk(config('filesystems.default'))->url($product->gallery[0]) }}"
                                            alt="imac image" />
                                    @else
                                        <img src="{{ asset('assets/placeholder.svg') }}" alt="Product1"
                                            class="w-20 h-20 object-cover rounded-md" />
                                    @endif
                                    <div class="flex-1">
                                        <h3 class="font-semibold text-gray-900">{{ $product->name }} ({{ $product->sku }} {{ $product->sort }})</h3>
                                        <p class="text-lg text-gray-500">
                                            {{ $product->new_price ? Number::format($product->new_price, locale: 'ru') . ' ₽' : ($product->price > 0 ? Number::format($product->getActualPrice(), locale: 'ru') . ' ₽' : 'По запросу') }}
                                        </p>
                                    </div>
                                    {{-- <p class="font-semibold text-gray-900 w-20 text-right">{{Number::format($product->price, 0)}} ₽
                                    </p> --}}
