            ...
            <form class="flex flex-col gap-4" wire:submit.prevent="save">
                @foreach ($fields as $field)
                    <label class="w-full">
                        ...
                        @switch($field['type'])
                            @case('text')
                                <input type="text" class="w-full bg-white rounded focus:bg-white focus:ring-2 text-base outline-none py-1 px-3 leading-8 transition-colors duration-200 ease-in-out @error($modelName) bg-red-50 border border-red-500 text-red-900 placeholder-red-700 @else border border-gray-300 focus:border-blue-500 focus:ring-blue-200 text-gray-700 @enderror" wire:model="{{ $modelName }}" @if($field['mask_enabled']) x-mask="{{ $field['mask'] }}" @endif wire:loading.attr="disabled" wire:target="save">
                                @break
                            @case('email')
                                <input type="email" class="w-full bg-white rounded border border-gray-300 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" wire:model="{{ $modelName }}" wire:loading.attr="disabled" wire:target="save">
                                @break

                            @case('textarea')
                                <textarea class="w-full bg-white rounded border border-gray-300 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out" wire:model="{{$modelName}}" wire:loading.attr="disabled" wire:target="save"></textarea>
                                @break
                            ...
                            @default
                                <input type="text" class="w-full bg-white rounded border border-gray-300 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" wire:model="{{$modelName}}" wire:loading.attr="disabled" wire:target="save">
                        @endswitch
                    </label>
                @endforeach
