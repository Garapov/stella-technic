...
class Items extends Component
{
    ...
    public function mount(
        ...
        try {
            ...
            if ($slug) {
                $this->category = ProductCategory::where(
                    "slug",
                    $slug
                )->first();
            ...
            if ($products) {
                // Здесь products - это массив ID вариантов товаров, а не товаров
                $this->product_ids = $products;
                $this->items = "variants"; // Добавляем маркер, что здесь ID вариантов, а не товаров
                \Illuminate\Support\Facades\Log::info(
                    "Установлены ID вариантов товаров",
                    [
                        "variants_count" => count($this->product_ids),
                    ]
                );
            }
            ...
            if ($this->priceFrom === null && $this->priceTo === null) {
                ...
                \Illuminate\Support\Facades\Log::info(
                    "Установлен диапазон цен по умолчанию",
                    [
            ...
            $this->dispatch(
                "filter-error",
                "Произошла ошибка при загрузке компонента: " . $e->getMessage()
            );
    ...
    public function handleFilterChanged()
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::info(
                "Событие filter-changed обработано",
                [
                    "selectedParams" => $this->selectedParams,
                    "selectedVariations" => $this->selectedVariations,
            ...
            $this->selectedVariations = array_map(
                "intval",
                (array) $this->selectedVariations
            );
            ...
            $this->priceFrom = is_numeric($this->priceFrom)
                ? (float) $this->priceFrom
                : null;
            $this->priceTo = is_numeric($this->priceTo)
                ? (float) $this->priceTo
                : null;
            $this->selectedBrands = array_map(
                "intval",
                (array) $this->selectedBrands
            );
            ...
            \Illuminate\Support\Facades\Log::error(
                "Ошибка в методе handleFilterChanged",
                [
                    "message" => $e->getMessage(),
                    "trace" => $e->getTraceAsString(),
                ]
            ...
            $this->dispatch(
                "filter-error",
                "Произошла ошибка при обновлении фильтра: " . $e->getMessage()
            );
        }
    }
    ...
    {
        \Illuminate\Support\Facades\Log::info(
            "Метод updatedSelectedVariations вызван",
            [
                "value" => $value,
                "selectedVariations" => $this->selectedVariations,
            ]
    ...
    {
        \Illuminate\Support\Facades\Log::info(
            "Метод updateSelectedParams вызван",
            [
                "selectedVariations" => $this->selectedVariations,
            ]
        );
        ...
        if (empty($this->selectedVariations)) {
            \Illuminate\Support\Facades\Log::info(
                "Нет выбранных вариаций, выходим"
            );
            return;
        }
        ...
        \Illuminate\Support\Facades\Log::info(
            "Результат группировки параметров",
            [
                "selectedParams" => $this->selectedParams,
            ]
        );
    ...
    public function updateParamSelection($paramName, $paramItemId)
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::info(
                "Метод updateParamSelection вызван",
                [
                    "paramName" => $paramName,
                    "paramItemId" => $paramItemId,
            ...
            if (!isset($this->selectedParams[$paramName])) {
                $this->selectedParams[$paramName] = [];
                \Illuminate\Support\Facades\Log::info(
                    "Создан новый массив для параметра",
                    [
                        "paramName" => $paramName,
                    ]
                );
            ...
            if ($index !== false) {
                ...
                $this->selectedParams[$paramName] = array_values(
                    $this->selectedParams[$paramName]
                );

                \Illuminate\Support\Facades\Log::info(
                    "Удален элемент параметра",
                    [
                        "paramName" => $paramName,
                        "paramItemId" => $paramItemId,
                        "paramValue" => $paramValue,
                ...
                if (empty($this->selectedParams[$paramName])) {
                    ...
                    \Illuminate\Support\Facades\Log::info(
                        "Удален пустой массив параметра",
                        [
                            "paramName" => $paramName,
                        ]
                    );
            ...
            \Illuminate\Support\Facades\Log::error(
                "Ошибка в методе updateParamSelection",
                [
                    "message" => $e->getMessage(),
                    "trace" => $e->getTraceAsString(),
                    "paramName" => $paramName,
            ...
            $this->dispatch(
                "filter-error",
                "Произошла ошибка при обновлении параметра фильтра: " .
                    $e->getMessage()
            );
    ...
    public function updatedPriceFrom($value)
    {
        ...
        if ($value > $this->priceTo) {
            ...
            \Illuminate\Support\Facades\Log::info(
                "Скорректирована нижняя граница цены",
                [
    ...
    public function updatedPriceTo($value)
    {
        ...
        if ($value < $this->priceFrom) {
            ...
            \Illuminate\Support\Facades\Log::info(
                "Скорректирована верхняя граница цены",
                [
    ...
    public function getProductsProperty()
    {
        try {
            \Illuminate\Support\Facades\Log::info(
                "Метод getProductsProperty вызван",
                [
                    "category" => $this->category,
                    "product_ids_count" => count($this->product_ids ?? []),
                    "items_type" => $this->items,
                    "selectedVariations" => $this->selectedVariations,
                    "selectedParams" => $this->selectedParams,
                    "priceFrom" => $this->priceFrom,
                    "priceTo" => $this->priceTo,
            ...
            if ($this->items === "variants") {
                ...
                $query = \App\Models\ProductVariant::whereIn(
                    "id",
                    $this->product_ids
                );
                \Illuminate\Support\Facades\Log::info(
            ...
            if ($this->items === "variants") {
                ...
                // Фильтрация по бренду, если выбраны бренды
                if (!empty($this->selectedBrands)) {
                    ...
                    \Illuminate\Support\Facades\Log::info(
                        "Применен фильтр по брендам к вариантам",
                        [
                            "selectedBrands" => $this->selectedBrands,
                ...
                \Illuminate\Support\Facades\Log::info("Получены товары", [
                    ...
                    "products_with_variants" => $products
                        ...
                        foreach ($product->variants as $variant) {
                            ...
                            if (!empty($this->selectedVariations)) {
                                ...
                                foreach (
                                    ...
                                    if (
                                        ...
                                        $variantMatches = false;
                                        \Illuminate\Support\Facades\Log::info(
                                            "Вариация не соответствует параметру",
                                            [
                                                "variant_id" => $variant->id,
                                                "variant_name" =>
                                                    $variant->name,
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    $variantMatches = false;
                                    \Illuminate\Support\Facades\Log::info(
                                        "Вариация не соответствует минимальной цене",
                                        [
                                            "variant_id" => $variant->id,
                                            "variant_name" => $variant->name,
                                            "variantPrice" => $variantPrice,
                                ...
                                if (
                                    ...
                                    $variantMatches = false;
                                    \Illuminate\Support\Facades\Log::info(
                                        "Вариация не соответствует максимальной цене",
                                        [
                                            "variant_id" => $variant->id,
                                            "variant_name" => $variant->name,
                                            "variantPrice" => $variantPrice,
                            ...
                            if ($variantMatches) {
                                \Illuminate\Support\Facades\Log::info(
                                    "Вариация добавлена в результаты",
                                    [
                                        "variant_id" => $variant->id,
                                        "variant_name" => $variant->name,
                                    ]
            ...
            \Illuminate\Support\Facades\Log::error(
                "Ошибка в методе getProductsProperty",
                [
                    "message" => $e->getMessage(),
                    "trace" => $e->getTraceAsString(),
                ]
    ...
    public function getAvailableBrandsProperty()
    {
        try {
            ...
            foreach ($variants as $variant) {
                // Проверяем работаем ли мы напрямую с вариантами или с товарами
                if ($this->items === "variants") {
                    // Если мы работаем с вариантами напрямую, убедимся, что связь с product загружена
                    ...
                    // Стандартная логика для товаров
                    if ($variant->product && $variant->product->brand_id) {
                        $activeBrandIds->push($variant->product->brand_id);
                    }
                }
            ...
            return $allBrands->map(function ($brand) use ($activeBrandIds) {
                $brand->would_have_results = $activeBrandIds->contains(
                    $brand->id
                );
            ...
            \Illuminate\Support\Facades\Log::error(
                "Ошибка в методе getAvailableBrandsProperty",
                [
                    "error" => $e->getMessage(),
                    "trace" => $e->getTraceAsString(),
                ]
    ...
    public function getPriceRangeProperty()
    {
        try {
            \Illuminate\Support\Facades\Log::info(
                "Начало метода getPriceRangeProperty"
            );
            ...
            // Если мы работаем напрямую с вариантами
            if ($this->items === "variants") {
                $variantsQuery = \App\Models\ProductVariant::whereIn(
                    "id",
                    $this->product_ids
                )->with("product.brand"); // Загружаем бренд для фильтрации

                // Применяем фильтр по брендам, если он выбран
                if (!empty($this->selectedBrands)) {
                    $variantsQuery->whereHas("product", function ($query) {
                        $query->whereIn("brand_id", $this->selectedBrands);
                    });
                }

                $variantsCollection = $variantsQuery->get();

                // Фильтруем по параметрам, если они выбраны
                if (!empty($this->selectedVariations)) {
                    foreach ($variantsCollection as $variant) {
                        ...
                        if (!$variant->relationLoaded("paramItems")) {
                            $variant->load("paramItems");
                        }
                        if (!$variant->relationLoaded("parametrs")) {
                            $variant->load("parametrs");
                        }
                    ...
                    // Если параметры не выбраны, добавляем все варианты
                ...
                if ($this->category) {
                    ...
                    $query = \App\Models\Product::whereIn(
                        "id",
                        $this->product_ids
                    );
                } else {
                ...
                if (!empty($this->selectedVariations)) {
                    ...
                    foreach ($products as $product) {
                        ...
                        foreach ($product->variants as $variant) {
                            ...
                            $variantParamIds = $variant->paramItems
                                ->pluck("id")
                                ->toArray();
            ...
            if ($variants->isEmpty()) {
                \Illuminate\Support\Facades\Log::info(
                    "Нет вариаций для расчета диапазона цен, используем значения по умолчанию"
                );
                return (object) [
                    "min_price" => 0,
                    "max_price" => 100000,
            ...
            \Illuminate\Support\Facades\Log::error(
                "Ошибка в методе getPriceRangeProperty",
                [
                    "error" => $e->getMessage(),
                    "trace" => $e->getTraceAsString(),
                ]
    ...
    public function getAvailableParamItemsProperty()
    {
        try {
            \Illuminate\Support\Facades\Log::info(
                "Начало метода getAvailableParamItemsProperty"
            );
            ...
            if (!$variants || $variants->isEmpty()) {
                \Illuminate\Support\Facades\Log::warning(
                    "Нет вариаций для получения параметров"
                );
                return collect();
            }

            \Illuminate\Support\Facades\Log::info(
                "Получены вариации для параметров",
                [
                    "variants_count" => $variants->count(),
            ...
            // Получаем все параметры для текущей категории или для переданных вариантов (без учета фильтров)
            $allCategoryParamItems = collect();

            if ($this->items === "variants") {
                // Если мы работаем напрямую с вариантами товаров
                $allVariants = \App\Models\ProductVariant::whereIn(
                    "id",
                    $this->product_ids
                )
                    ->with([
                ...
                // Собираем все параметры из всех вариаций
            ...
            } elseif ($this->product_ids && $this->items !== "variants") {
                // Получаем все товары по ID без фильтров
                $allProducts = \App\Models\Product::whereIn(
                    "id",
                    $this->product_ids
                )
                    ->with([
            ...
            foreach ($variants as $variant) {
                if (!$variant) {
                    \Illuminate\Support\Facades\Log::warning(
                        "Обнаружена null вариация"
                    );
                    continue;
                }

                \Illuminate\Support\Facades\Log::info(
                    "Обработка вариации для параметров",
                    [
                        "variant_id" => $variant->id,
                        "variant_name" => $variant->name,
                        "has_param_items" =>
                            $variant->paramItems &&
                            $variant->paramItems->count() > 0,
                        "param_items_count" => $variant->paramItems
                            ? $variant->paramItems->count()
                            : 0,
                    ]
                );
                ...
                if ($variant->paramItems) {
                    foreach ($variant->paramItems as $paramItem) {
                        ...
                        \Illuminate\Support\Facades\Log::info(
                            "Добавление параметра вариации (paramItems)",
                            [
                                "param_id" => $param->id,
                ...
                if ($variant->parametrs) {
                    foreach ($variant->parametrs as $paramItem) {
                        ...
                        \Illuminate\Support\Facades\Log::info(
                            "Добавление параметра вариации (parametrs)",
                            [
                                "param_id" => $param->id,
                                "param_name" => $param->name,
                                "param_item_id" => $paramItem->id,
            ...
            if ($allCategoryParamItems->isNotEmpty()) {
                $allCategoryParamItems = $allCategoryParamItems->unique(
                    function ($item) {
                ...
                foreach ($allCategoryParamItems as $item) {
                    ...
                    if (
                        !isset(
                            $paramGroups[$item["param_id"]]["items"][
                                $item["id"]
                            ]
                        )
                    ) {
                        $paramGroups[$item["param_id"]]["items"][
                            ...
                            "would_have_results" => $activeParamItemIds->contains(
                                $item["id"]
                            ),
                        ];
                    }
            ...
            if (!empty($this->selectedVariations)) {
                $selectedItems = \App\Models\ProductParamItem::whereIn(
                    "id",
                    $this->selectedVariations
                )
                    ->with("productParam")
            ...
            \Illuminate\Support\Facades\Log::info(
                "Результат getAvailableParamItemsProperty",
                [
                    "param_groups_count" => $paramItems->count(),
                    "param_groups" => $paramItems->pluck("name")->toArray(),
                    "param_items_counts" => $paramItems
                        ->map(function ($group) {
                            return [
                                "group" => $group["name"],
                                "items_count" => $group["items"]->count(),
                                "active_items_count" => $group["items"]
                                    ->where("would_have_results", true)
                                    ->count(),
                                "items" => $group["items"]
                                    ->pluck("title")
                                    ->toArray(),
            ...
            \Illuminate\Support\Facades\Log::error(
                "Ошибка в методе getAvailableParamItemsProperty",
                [
                    "error" => $e->getMessage(),
                    "trace" => $e->getTraceAsString(),
                ]
            ...
            $this->dispatch("error", [
                "message" =>
                    "Произошла ошибка при получении доступных параметров: " .
                    $e->getMessage(),
            ]);
    ...
    public function getAvailableFiltersProperty()
    {
        try {
            \Illuminate\Support\Facades\Log::info(
                "Начало метода getAvailableFiltersProperty"
            );
            ...
            \Illuminate\Support\Facades\Log::info(
                "Получены доступные параметры для фильтров",
                [
                    "param_groups_count" => $paramItems->count(),
                    "param_groups" => $paramItems->pluck("name")->toArray(),
                ]
            ...
            foreach ($paramItems as $paramGroup) {
                \Illuminate\Support\Facades\Log::info(
                    "Обработка группы параметров для фильтров",
                    [
            ...
            \Illuminate\Support\Facades\Log::info(
                "Получены доступные бренды для фильтров",
                [
            ...
            \Illuminate\Support\Facades\Log::info(
                "Результат getAvailableFiltersProperty",
                [
            ...
            \Illuminate\Support\Facades\Log::error(
                "Ошибка в методе getAvailableFiltersProperty",
                [
                    "error" => $e->getMessage(),
                    "trace" => $e->getTraceAsString(),
                ]
            ...
            $this->dispatch("error", [
                "message" =>
                    "Произошла ошибка при получении доступных фильтров: " .
                    $e->getMessage(),
            ]);
        ...
        try {
            ...
            try {
                ...
                if (!$filters) {
                    $filters = collect();
                    \Illuminate\Support\Facades\Log::warning(
                        "Фильтры не найдены, используем пустую коллекцию"
                    );
                }

                \Illuminate\Support\Facades\Log::info(
                    "Получены фильтры для отображения",
                    [
                        "filters_count" => $filters->count(),
                        "filters" => $filters
                            ->map(function ($filter) {
                                return [
                                    ...
                                    "items_count" => isset($filter["items"])
                                        ? count($filter["items"])
                                        : (isset($filter->params)
                                            ? $filter->params->count()
                                            : 0),
            ...
            try {
                ...
                if (!$products) {
                    $products = collect();
                    \Illuminate\Support\Facades\Log::warning(
                        "Товары не найдены, используем пустую коллекцию"
                    );
                }

                \Illuminate\Support\Facades\Log::info(
                    "Получены товары для отображения",
                    [
                        "products_count" => $products->count(),
            ...
            try {
                ...
                \Illuminate\Support\Facades\Log::error(
                    "Ошибка при получении опций сортировки",
                    [
                        "error" => $e->getMessage(),
                        "trace" => $e->getTraceAsString(),
                    ]
    ...
    public function dehydrate()
    {
        try {
            ...
            $this->selectedVariations = array_map(
                "intval",
                array_values(array_filter((array) $this->selectedVariations))
            );

            // Преобразуем selectedParams в массив, чтобы избежать проблем с сериализацией
            $params = [];
            foreach ((array) $this->selectedParams as $key => $value) {
                $params[$key] = array_map(
                    "intval",
            ...
            $this->selectedBrands = array_map(
                "intval",
                array_values(array_filter((array) $this->selectedBrands))
            );
            ...
            $this->dispatch(
                "filter-error",
                "Произошла ошибка при сериализации компонента: " .
                    $e->getMessage()
            );
    ...
    public function hydrate()
    {
        try {
            ...
            $this->dispatch(
                "filter-error",
                "Произошла ошибка при десериализации компонента: " .
                    $e->getMessage()
            );
